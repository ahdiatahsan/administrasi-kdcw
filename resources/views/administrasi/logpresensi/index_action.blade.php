...
<form action="{{ route('presensi.destroy', $presensi->id) }}" method="POST">
    ...
    <span class="dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-right"
            ...
            <a class="dropdown-item" href="{{ route('presensi.edit', $presensi->id) }}"><i
                    class="la la-edit"></i>Ubah</a>
            <button type="button" class="dropdown-item" data-toggle="modal"
                data-target="#delete_modal-{{ $presensi->id }}">
                <i class="la la-trash"></i>Hapus
            </button>
        </div>
    </span>

    <a href="{{ route('presensi.show', $presensi->id) }}" class="btn btn-sm btn-clean btn-icon btn-icon-md"
        title="Detail">
        <i class="fa fa-search text-brand"></i>
    </a>

    <!-- Modal -->
    <div class="modal fade" id="delete_modal-{{ $presensi->id }}" tabindex="-1" role="dialog" aria-hidden="true">
...
<a href="{{ route('presensi.show', $presensi->id) }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Detail">
    <i class="fa fa-search text-brand"></i>
</a>
@endif